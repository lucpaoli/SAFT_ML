...
# @show device = Flux.get_device(; verbose=true)
# @show device.deviceID
...
train_loader = DataLoader(train_data, batchsize=32, shuffle=true) |> gpu
test_loader = DataLoader(test_data, batchsize=32, shuffle=false) |> gpu
...
# @show typeof(unbounded_model[1].W)
@show unbounded_model[1].weight
