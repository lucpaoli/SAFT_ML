...
function main(; epochs=5000)
    train_loader, test_loader = create_data(n_points=60, batch_size=800, shuffle_train=false)
    ...
    train_loader, test_loader = create_data(n_points=60, batch_size=800, shuffle_train=true)
    optim = Flux.setup(Flux.Descent(1e-3), model)
