...
function create_ff_model(nfeatures)
    ...
    model = Chain(
        Dense(nfeatures, nout * 8, relu, init=Flux.glorot_normal),
        Dense(nout * 8, nout * 4, relu, init=Flux.glorot_normal),
        # MultiHeadAttention(nout * 4; nheads=4, init=Flux.glorot_uniform),
        Dense(nout * 4, nout * 2, relu, init=Flux.glorot_normal),
...
function train_model!(model, train_loader, test_loader; epochs=10, log_filename="params_log_linear_alkanes_sgd.csv")
