...
# 3:2 weighting for pressure:density
function eval_loss(X_batch, y_batch, metric, model, use_saft_head)
    ...
    for (X, y_vec) in zip(X_batch, y_batch)
        ...
        if use_saft_head
            for (y, ŷ, w) in zip(y_vec, ŷ_vec, weighting)
                if isnothing(ŷ)
                    n_failed += 1
                    continue
                end
                batch_loss += w*metric(y, ŷ)
                n += 1
            end
        else
...
function create_X_data(model, mol_dict, batch_mols, x0_cache, Tc0_cache; pretraining=false)
    ...
    # for i in 1:length(batch_mols)
        ...
        elseif abs(deriv/deriv2) > 1e-1
            println("abs(∂²A∂V²/∂³A∂V²∂T) > 0.1, = $deriv, Tc=$Tc for $mol, skipping...")
            continue
        end
...
function main_pcpsaft()
    ...
    # Create a new final layer with desired initialization
    new_final_layer = Dense(32, 4, x -> x)

    # Rebuild the model with the new final layer
    new_model = Chain(layers..., new_final_layer)
