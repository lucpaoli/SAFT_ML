...
function train_model!(model, mol_dict, optim; epochs=1000, n_batches=5, pretraining=false)
    ...
    Tc0_cache = Dict{String,Union{Nothing,Vector{BigFloat}}}(mol => nothing for mol in training_molecules)
...
function main()
    ...
    optim = Flux.setup(Flux.Adam(5e-3), model)
    train_model!(model, mol_dict, optim; epochs=10000, n_batches=5, pretraining=false)
end
