...
function create_data(; batch_size=16, n_points=25, pretraining=false)
    ...
    #! This is a data leak - need to apply this mask based on just training molecules!
    for num in [0, 1]
...
function SAFT_head(model, X)
    ...
    @assert abs(Tc2 - Tc) < 1e-6 "Temperature newton step failed, Tc2 = $Tc2, Tc = $Tc, $(∂²A∂V²(saft_params, Vc, Tc)) $(∂³A∂V²∂T(saft_params, Vc, Tc))"
...
function create_X_data(model, mol_dict, batch_mols, x0_cache, Tc0_cache)
    ...
    # for i in 1:length(batch_mols)
        ...
        # n > 1 && @assert false
        # n += 1
        # @show Tc, ∂²A∂V²(saft_params, Vc, Tc), ∂³A∂V²∂T(saft_params, Vc, Tc), saft_params
...
function train_model!(model, mol_dict, optim; epochs=1000, n_batches=5, pretraining=false)
    ...
    all_mols = collect(keys(mol_dict))
    shuffle!(all_mols)
    train_mols, val_mols = splitobs(all_mols, at=0.8)

    batch_size = ceil(Int, length(train_mols) / n_batches)
    train_loader = DataLoader(train_mols; batchsize=batch_size)
    # val_loader = DataLoader(val_mols; batchsize=batch_size)
...
function main()
    ...
    optim = Flux.setup(Flux.Optimiser(Flux.Adam(), ExpDecay(η = 1e-4, decay = 0.1, decay_step = 1000, clip = 1e-7, start = 1)), model)
    train_model!(model, mol_dict, optim; epochs=10000, n_batches=4, pretraining=false)
end
...
function main_pcpsaft()
    ...
    train_model!(model, mol_dict, optim; epochs=25, n_batches=4, pretraining=true)
