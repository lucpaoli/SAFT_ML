...
function create_ff_model_with_attention(nfeatures)
    nout = 4
    compressed_dim = 128
    attention_dim = compressed_dim  # Assuming the attention layer follows the first Dense layer
    ...
    return Chain(
        Dense(nfeatures, nfeatures*2, relu),
        Dense(nfeatures*2, compressed_dim, relu),
        attention_wrapper,
        Dense(compressed_dim, nout, x -> x),
    )
end
