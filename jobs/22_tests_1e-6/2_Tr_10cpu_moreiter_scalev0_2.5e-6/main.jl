...
function create_data(model, mol_dict, batch_mols, x0_cache, Tc0_cache)
    ...
    Threads.@threads for i in 1:length(batch_mols)
        ...
        for (j, (Tr, Y)) in enumerate(zip(X_vec, Y_vec))
            T = Tr * Tc

            if isnothing(x0)
                method = ChemPotVSaturation(crit=(Tc, pc, Vc), max_iters=50_000)
            else
                method = ChemPotVSaturation(vl=x0[1], vv=x0[2], crit=(Tc, pc, Vc), max_iters=50_000)
            end
            ...
            if Vₗ == Vᵥ
                ...
                x0 = nothing
            elseif isnan(p_sat)
                mol_sat_solves_failed == 0 && println("Sat solver failed for $mol = $(round.(saft_params, digits=2)) at T=$(round(T, digits=2)), Tc=$(round(Tc, digits=2)), muting...")
...
function main_pcpsaft()
    ...
    optim = Flux.setup(Flux.Adam(2.5e-6), model)
