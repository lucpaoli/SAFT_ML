...
function eval_loss(X_batch, y_batch, metric, model; use_saft_head=true)
    batch_loss = 0.0
    n = 0
...
function train_model!(model, train_loader, test_loader, optim; epochs=10, log_filename="params_log.csv", pretraining=false)
...
function main_pcpsaft(; epochs=1000)
    train_loader, test_loader = create_data(n_points=50, batch_size=, pretraining=true)
    @show n_features = length(first(train_loader)[1][1][1])
